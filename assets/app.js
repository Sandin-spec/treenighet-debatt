...
function setDetail(v){
  ...
  // Mobil: när man valt en vers, scrolla direkt till detaljkortet
  if (window.matchMedia && window.matchMedia("(max-width:980px)").matches){
    const card = document.getElementById("detailCard") || detailEl.closest(".card");
    if(card){
...
async function init(){
  ...
  // Mobilhjälp: snabb tillbaka till listan utan att behöva scrolla manuellt
  if(backToListBtn){
    backToListBtn.addEventListener("click", () => {
      const listCard = document.getElementById("listCard") || document.querySelector(".card");
      if(listCard){
